...
def tensortest():
    ...
    if(class_name == class_names[1]):
        class_name = "Poopsie"
    if(class_name == class_names[2]):
        class_name = "Tractor"
    if(class_name == class_names[3]):
        class_name = "Mario"
    if(class_name == class_names[4]):
        class_name = "Kiwi"
    if(class_name == class_names[5]):
        class_name = "Rubiks"
    if(class_name == class_names[6]):
        class_name = "Jumbo"
...
class SimplePublisher(Node):
    ...
    def timer_callback(self):
        ...
        #! change these values for each object
        #! you can also try to just add the camrea lens distance for the smaller objects
        #! you can also try just making it move one more time before turning whihc might work
        tensordata = tensortest()
        print(tensordata[1])
        if(tensordata[1] == "Mug" and tensordata[0]>0.99996 ):
            goleft90()
            count = count+1
        elif(tensordata[1] == "Poopsie" and tensordata[0]>0.9994):
            goright90()
            count = count+1
        elif(tensordata[1] == "Tractor" and tensordata[0]>0.99):
            goleft90()
            count = count+1
        elif(tensordata[1] == "Kiwi" and tensordata[0]>0.9994):
            goleft90()
            count = count+1
        elif(tensordata[1] == "Mario" and tensordata[0]>0.9995):
            goright90()
            count = count+1
        elif(tensordata[1] == "Jumbo" and tensordata[0]>0.95):
            goleft90()
            goleft90()
            goleft90()
            goleft90()
            
        elif(tensordata[1] == "Rubiks" and tensordata[0]>0.995):
            goright90()
            count = count+1
